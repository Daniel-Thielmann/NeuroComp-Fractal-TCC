# graphics/confusion_matrix_comparison.py

import os
import pandas as pd
import numpy as np
import matplotlib.pyplot as plt
import seaborn as sns
from sklearn.metrics import confusion_matrix

# ================================
# Utilitário para carregar labels e predições
# ================================
def load_predictions_and_labels(method):
    base_dir = f"results/{method}"
    probs, labels, preds = [], [], []

    for phase in ['Training', 'Evaluate']:
        folder = os.path.join(base_dir, phase)
        for file in sorted(os.listdir(folder)):
            if file.endswith(".csv"):
                df = pd.read_csv(os.path.join(folder, file))
                df = df[df['true_label'].isin([1, 2])]
                labels.extend(df['true_label'])

                predicted = np.where(df['left_prob'] > df['right_prob'], 1, 2)
                preds.extend(predicted)

    return np.array(labels), np.array(preds)

# ================================
# Geração do Gráfico
# ================================
def plot_confusion_matrices():
    # Carrega os dados
    labels_h, preds_h = load_predictions_and_labels("Higuchi")
    labels_l, preds_l = load_predictions_and_labels("LogPower")

    # Gera as matrizes de confusão
    cm_h = confusion_matrix(labels_h, preds_h, labels=[1, 2])
    cm_l = confusion_matrix(labels_l, preds_l, labels=[1, 2])

    # Cria gráfico lado a lado
    fig, axes = plt.subplots(1, 2, figsize=(10, 4))

    sns.heatmap(cm_h, annot=True, fmt='d', cmap='Blues',
                xticklabels=['Left', 'Right'], yticklabels=['Left', 'Right'], ax=axes[0])
    axes[0].set_title('Higuchi')
    axes[0].set_xlabel('Predito')
    axes[0].set_ylabel('Real')

    sns.heatmap(cm_l, annot=True, fmt='d', cmap='Oranges',
                xticklabels=['Left', 'Right'], yticklabels=['Left', 'Right'], ax=axes[1])
    axes[1].set_title('LogPower')
    axes[1].set_xlabel('Predito')
    axes[1].set_ylabel('')

    plt.suptitle("Matriz de Confusão por Método - Higuchi vs LogPower", fontsize=14)
    plt.tight_layout(rect=[0, 0, 1, 0.95])

    os.makedirs("graphics", exist_ok=True)
    output_path = "graphics/confusion_matrix_comparison.png"
    plt.savefig(output_path, dpi=300)
    print(f"Matriz de confusão salva em: {output_path}")
    plt.close()


# ================================
# Execução
# ================================
if __name__ == "__main__":
    plot_confusion_matrices()
