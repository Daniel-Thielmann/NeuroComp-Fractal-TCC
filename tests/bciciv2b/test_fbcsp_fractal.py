...
def extract_fbcsp_fractal_features(X, y, frequency_bands, n_components=2, kmax=10):
    """Extrai features FBCSP + Fractal para BCICIV2b (3 canais)."""
    all_features = []
    ...
    for low_freq, high_freq in frequency_bands:
        ...
        # Calcula features fractais para cada componente CSP
        band_features = []
        for trial in selected_comps:
            ...
            for comp in trial:
                ...
                fractal_result = higuchi_fractal(comp_data, flating=True, kmax=kmax)
                fractal_dim = fractal_result["X"][0, 0]
                # Features adicionais (energia e desvio padrão)
...
def test_fbcsp_fractal_classification_bciciv2b():
    print('Teste "FBCSP + Fractal" ("BCICIV2b"):')
    ...
    for subject_id in range(1, 10):
        try:
            ...
            if X.shape[0] < 10:
                continue
            features = extract_fbcsp_fractal_features(
                X, y, frequency_bands, n_components=2, kmax=10
            )
            ...
            for fold_idx, (train_idx, test_idx) in enumerate(skf.split(features, y)):
                ...
                mibif = SelectKBest(
                    score_func=mutual_info_classif, k=min(15, features.shape[1])
                )
            ...
            os.makedirs("results/BCICIV2b/fbcsp_fractal/evaluate", exist_ok=True)
            os.makedirs("results/BCICIV2b/fbcsp_fractal/training", exist_ok=True)
            eval_df.to_csv(
                f"results/BCICIV2b/fbcsp_fractal/evaluate/P{subject_id:02d}_evaluate.csv",
                index=False,
            )
            train_df.to_csv(
                f"results/BCICIV2b/fbcsp_fractal/training/P{subject_id:02d}_training.csv",
                index=False,
            )
            print(
    ...
    print(f'Resumo "FBCSP + Fractal" ("BCICIV2b"):')
    print(
        f"Acc média={np.mean(all_accuracies):.4f}±{np.std(all_accuracies):.4f} | Kappa média={np.mean(all_kappas):.4f}±{np.std(all_kappas):.4f}"
    )
    print("CSV salvo: results/BCICIV2b/fbcsp_fractal/")
    return results


if __name__ == "__main__":
    try:
        test_fbcsp_fractal_classification_bciciv2b()
    except Exception:
        sys.exit(1)
