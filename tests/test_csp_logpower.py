...
def test_run_csp_logpower_format():
    """Testa o formato de saída da função run_csp_logpower para um sujeito."""
    # Executa o CSP+LogPower para um sujeito
    subject_id = 1
...
def test_run_csp_logpower_performance():
    """Testa o desempenho do método CSP+LogPower em um sujeito."""
    # Executa o CSP+LogPower para um sujeito
    subject_id = 1
    rows = run_csp_logpower(subject_id)
    ...
    print(f"Acurácia do CSP+LogPower para o sujeito {subject_id}: {accuracy:.4f}")


def run_csp_logpower_all_subjects():
    """Executa o método CSP+LogPower para todos os sujeitos e retorna os resultados."""
    all_rows = []
    ...
    for subject_id in tqdm(range(1, 10), desc="CSP_LogPower"):
        try:
            ...
            os.makedirs("results/CSP_LogPower/Evaluate", exist_ok=True)
            
            df_subject[df_subject["fold"] < 4].to_csv(
                f"results/CSP_LogPower/Training/P{subject_id:02d}.csv", index=False
            )
            df_subject[df_subject["fold"] == 4].to_csv(
                f"results/CSP_LogPower/Evaluate/P{subject_id:02d}.csv", index=False
            )
...
def test_csp_logpower_all_subjects_performance():
    """Testa o desempenho do método CSP+LogPower em todos os sujeitos."""
    df = run_csp_logpower_all_subjects()
...
if __name__ == "__main__":
    ...
    test_run_csp_logpower_format()
    test_run_csp_logpower_performance()
    print("Todos os testes unitários passaram!")
    
    # Executa o teste de desempenho para todos os sujeitos
    test_csp_logpower_all_subjects_performance()
