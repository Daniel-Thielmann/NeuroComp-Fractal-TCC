...
def test_fractal_classification_wcci2020():
    print('Teste "Fractal" ("WCCI2020"):')
    results = {}
    all_accuracies = []
    all_kappas = []
    os.makedirs("results/wcci2020/fractal/evaluate", exist_ok=True)
    os.makedirs("results/wcci2020/fractal/training", exist_ok=True)
    for subject_id in range(1, 10):
        try:
            ...
            if X.shape[0] < 10:
                continue
            X_filtered = chebyshev2_bandpass(X, 4, 40, fs=sfreq)
            ...
            eval_df.to_csv(
                f"results/wcci2020/fractal/evaluate/P{subject_id:02d}_evaluate.csv",
                index=False,
            )
            train_df = pd.DataFrame(fold_train_results)
            train_df.to_csv(
                f"results/wcci2020/fractal/training/P{subject_id:02d}_training.csv",
                index=False,
            )
            print(
    ...
    if summary_data:
        ...
        summary_df.to_csv(
            "results/wcci2020/fractal/fractal_classification_results.csv", index=False
        )
        print("CSV salvo: results/wcci2020/fractal/")
    return results


if __name__ == "__main__":
    try:
        test_fractal_classification_wcci2020()
    except Exception as e:
        print(f"Erro: {str(e)}")
        sys.exit(1)
