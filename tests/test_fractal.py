...
def test_higuchi_fractal_feature():
    """
    Testa a função higuchi_fractal com dados simulados.
    ...
    """
    print("Testando função higuchi_fractal...")
    ...
    result = higuchi_fractal(eegdata, flating=True)

    # Verificações de integridade
    assert "X" in result, "Resultado deve conter chave 'X'"
    assert result["X"].shape == (50, 12), f"Shape incorreto: {result['X'].shape}"
    ...
    assert np.all(result["X"] > 0), "Todas as dimensões fractais devem ser positivas"

    # Estatísticas descritivas
    mean_hfd = np.mean(result["X"])
    std_hfd = np.std(result["X"])
    min_hfd = np.min(result["X"])
    max_hfd = np.max(result["X"])

    print(f"Teste concluído com sucesso.")
    print(f"Estatísticas da dimensão fractal:")
    print(f"  Média: {mean_hfd:.4f}")
    print(f"  Desvio padrão: {std_hfd:.4f}")
    print(f"  Mínimo: {min_hfd:.4f}")
    print(f"  Máximo: {max_hfd:.4f}")

    return True


def test_fractal_classification_wcci2020():
    """
    Testa classificação usando features Higuchi Fractal em todos os sujeitos do WCCI2020.
    ...
    """
    print("Testando classificação com Higuchi Fractal no dataset WCCI2020...")
    print("Dataset: WCCI2020 (9 sujeitos)")
    print("Tarefa: Classificação de motor imagery usando features fractais")
    print(
        "Pipeline: Filtro 8-30Hz -> Higuchi Fractal -> Normalização -> LDA (5-fold CV)"
    )
    print("-" * 70)
    ...
    for subject_id in range(1, 10):  # Sujeitos 1-9
        ...
        try:
            ...
            # Extrai features fractais dos dados filtrados
            eegdata = {"X": X_filtered}
            fractal_result = higuchi_fractal(eegdata, flating=True)
            features = fractal_result["X"]  # [n_trials, n_channels]
    ...
    print("Teste de classificação com Higuchi Fractal concluído com sucesso.")

    return results


if __name__ == "__main__":
    print("=== TESTE: Função Higuchi Fractal ===")

    # Teste básico com dados simulados
    print("\n1. Teste básico com dados simulados:")
    success = test_higuchi_fractal_feature()
    if success:
    ...
    try:
        results = test_fractal_classification_wcci2020()
        ...
        if summary_data:
            ...
            summary_df.to_csv(
                "results/test_outputs/fractal_wcci2020_classification_results.csv",
                index=False,
            )
            print(
                f"\n✓ Resultados salvos em: results/test_outputs/fractal_wcci2020_classification_results.csv"
            )
