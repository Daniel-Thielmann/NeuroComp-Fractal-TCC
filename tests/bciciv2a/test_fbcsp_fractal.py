...
def extract_fbcsp_fractal_features(X, y, frequency_bands, n_components=4, kmax=10):
    """Extrai features FBCSP + Fractal."""
    all_features = []
...
def test_fbcsp_fractal_classification_bciciv2a():
    print('Teste "FBCSP + Fractal" ("BCICIV2a"):')
    frequency_banks = [
        (4, 8),
        (8, 12),
        (12, 16),
    ...
    for subject_id in range(1, 10):
        try:
            ...
            features = extract_fbcsp_fractal_features(
                X, y, frequency_banks, n_components=4, kmax=10
            )
...
if __name__ == "__main__":
    print("Teste FBCSP + Fractal (BCICIV2a)")
    try:
        results = test_fbcsp_fractal_classification_bciciv2a()
        summary_data = []
        ...
        if summary_data:
            ...
            summary_df.to_csv(
                "results/BCICIV2a/fbcsp_fractal/fbcsp_fractal_classification_results.csv",
                index=False,
            )
            print(
                "CSV salvo: results/BCICIV2a/fbcsp_fractal/fbcsp_fractal_classification_results.csv"
            )
    except Exception as e:
        print(f"Erro: {str(e)}")
        sys.exit(1)
