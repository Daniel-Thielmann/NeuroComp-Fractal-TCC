...
def test_fbcsp_logpower_classification_bciciv2a():
    ...
    for subject_id in range(1, 10):
        try:
            ...
            os.makedirs("results/BCICIV2a/fbcsp_logpower/evaluate", exist_ok=True)
            os.makedirs("results/BCICIV2a/fbcsp_logpower/training", exist_ok=True)
            eval_df = pd.DataFrame(fold_results)
            eval_df.to_csv(
                f"results/BCICIV2a/fbcsp_logpower/evaluate/P{subject_id:02d}_evaluate.csv",
                index=False,
            )
            train_df = pd.DataFrame(fold_train_results)
            train_df.to_csv(
                f"results/BCICIV2a/fbcsp_logpower/training/P{subject_id:02d}_training.csv",
                index=False,
            )
            mean_accuracy = np.mean(fold_accuracies)
            std_accuracy = np.std(fold_accuracies)
